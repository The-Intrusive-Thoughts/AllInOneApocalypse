...
public final class AioaGraphUpdateHandler {
    ...
    public static void handle(ServerPlayer player, AioaGraphUpdateRequest request) {
        ...
        try {
            ...
            if (!issues.isEmpty()) {
                player.sendOverlayMessage(Component.literal("AIOA graph rejected: " + issues.get(0)));
                return;
            }
            if (graph.scope != AioaBehaviorGraph.Scope.SINGLE_ENTITY
                    && !player.level().getServer().getPlayerList().isOp(player.nameAndId())) {
                player.sendOverlayMessage(Component.literal("Operator permission is required for non-instance behavior graphs."));
                return;
            }
            ...
            player.sendOverlayMessage(Component.literal("Applied behavior graph: " + graph.name));
        } catch (RuntimeException exception) {
            player.sendOverlayMessage(Component.literal("AIOA graph data was invalid."));
        }
    }
    ...
    private static void handleWorkspace(ServerPlayer player, WorkspaceUpdate update) {
        if (update == null || update.version < 1 || update.version > 2 || update.graphs == null || update.graphs.size() > 128) {
            player.sendOverlayMessage(Component.literal("AIOA workspace data was invalid."));
            return;
        }
        ...
        for (AioaBehaviorGraph graph : graphs) {
            List<String> issues = AioaBehaviorValidator.validate(graph);
            if (!issues.isEmpty()) {
                if (!update.silent) player.sendOverlayMessage(Component.literal("AIOA graph rejected: " + issues.get(0)));
                return;
            }
        }
        ...
        AioaConfigManager.save(config);
        if (!update.silent) player.sendOverlayMessage(Component.literal("Applied " + graphs.size() + " behavior graph(s)."));
    }
