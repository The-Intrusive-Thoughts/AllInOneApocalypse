...
public final class AioaBehaviorRuntime {
    ...
    private static void spawnMob(AioaBehaviorGraph.Node node, Mob source) {
        ...
        spawned.finalizeSpawn(level, level.getCurrentDifficultyAt(spawned.blockPosition()), MobSpawnType.EVENT, null, null);
    ...
    private static void playSound(AioaBehaviorGraph.Node node, Mob mob) {
        ...
        SoundEvent sound = id.map(BuiltInRegistries.SOUND_EVENT::get).orElse(null);
        if (sound != null) {
